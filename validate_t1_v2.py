...
def minute_eastmoney(date,code):
    ...
    for x in seq:
        ...
        rows.append({'trade_time':pd.Timestamp(a[0]),'close':float(a[2]),'high':float(a[3]),'low':float(a[4]),'vol':float(a[5]),'amount':float(a[6])})
...
def main():
    ...
    summary={'total_samples':len(out),'completed_samples':len(c),'not_completed_samples':int((out.status!='completed').sum()),'profit_samples':int((c.net_return_pct>0).sum()),'non_loss_samples':int((c.net_return_pct>=0).sum()),'loss_samples':int((c.net_return_pct<0).sum()),'win_rate_pct':float((c.net_return_pct>0).mean()*100),'non_loss_rate_pct':float((c.net_return_pct>=0).mean()*100),'average_net_return_pct':float(c.net_return_pct.mean()),'median_net_return_pct':float(c.net_return_pct.median()),'max_net_return_pct':float(c.net_return_pct.max()),'min_net_return_pct':float(c.net_return_pct.min())}
