...
BUY_BPS=2.5+2.0; SELL_BPS=2.5+5.0+2.0
...
def minute_eastmoney(date,code):
    url=('https://push2his.eastmoney.com/api/qt/stock/trends2/get?secid='+secid(code)+
         '&ndays=5&iscr=0&iscca=0&ut=fa5fd1943c7b386f172d6893dbfba10b&fields1=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13'
         '&fields2=f51,f52,f53,f54,f55,f56,f57,f58')
    j=req_json(url); data=j.get('data') or {}; seq=data.get('trends') or []
    rows=[]
    for x in seq:
        a=x.split(',')
        if len(a)<8 or not a[0].startswith(f'{date[:4]}-{date[4:6]}-{date[6:]}'): continue
...
def board_info(d):
    ...
    for i in range(ft+1,len(d)):
        if not opened and bool(d.iloc[i-1].on) and bool(d.iloc[i].below):
            seg+=1; opened=True; fo=i if fo is None else fo
...
def exit_rule(d,entry,pre,up):
    ...
    if op<=stop: si=fi=0; ft=w.iloc[0].trade_time; fp=op; reason='gap_below_hard_stop'
        ...
        for i,r in w.iterrows():
            ...
            if mode=='rally_or_high_level_oscillation':
                ...
                if i+1>=MIN_OBS and (since>=STALL or pb<=-PULLBACK):
                    si=i;fi,ft,fp=next_fill(w,i);reason='first_rally_stalled' if since>=STALL else 'first_rally_reversed';break
            else:
                ...
                if started:
                    ...
                    if since>=STALL or pb<=-PULLBACK:
                        si=i;fi,ft,fp=next_fill(w,i);reason='first_rebound_stalled' if since>=STALL else 'first_rebound_failed';break
    ...
    win=w.iloc[:fi+1]; hi_i=int(win.high.idxmax()); wh=float(w.iloc[hi_i].high); wl=float(win.low.min())
    net=((fp*(1-SELL_BPS/10000))/(entry*(1+BUY_BPS/10000))-1)*100
...
def main():
    ...
    for n,s in samples.iterrows():
        ...
        try:
            sday=pick(daily(sd),code); spre=float(sday.pre_close); sclose=float(sday.close); sup=float(pick(limits(sd),code).up_limit)
            r.update(signal_pre_close=spre,signal_close=sclose,signal_up_limit=sup)
            try:
                tday=pick(daily(td),code); tpre=float(tday.pre_close); tup=float(pick(limits(td),code).up_limit)
            except Exception:
                tpre=sclose; tup=round(tpre*(1+limit_ratio(code,name))+1e-10,2)
            r.update(t1_pre_close=tpre,t1_up_limit=tup)
            try:
                sm,ssrc=minute(sd,code); si=indicators(sm,spre,sup); seg,ft,fo,rs=board_info(si); ei,ep,es=entry_from_minutes(si,fo)
                r.update(signal_minute_source=ssrc,signal_first_touch=si.iloc[ft].trade_time.strftime('%H:%M') if ft is not None else '',signal_open_segments_1m=seg,signal_first_open=si.iloc[fo].trade_time.strftime('%H:%M') if fo is not None else '',signal_reseal=si.iloc[rs].trade_time.strftime('%H:%M') if rs is not None else '')
                if ep is None: ep=sclose; es='conservative_signal_close_after_no_fill'; et='15:00'
                else: et=si.iloc[ei].trade_time.strftime('%H:%M')
            except Exception as e:
                ep=sclose; es='conservative_signal_close_no_tday_1m'; et='15:00'; r['signal_data_note']=str(e)
            r.update(entry_source=es,entry_time=et,entry_price=ep)
            tm,tsrc=minute(td,code); ti=indicators(tm,tpre,tup); r['t1_minute_source']=tsrc; r.update(exit_rule(ti,float(ep),tpre,tup)); r['status']='completed'
        except Exception as e:r['detail']=repr(e)
        rows.append(r)
    out=pd.DataFrame(rows); out.to_csv(OUT/'all_35_results.csv',index=False,encoding='utf-8-sig')
    c=out[out.status=='completed'].copy(); c['net_return_pct']=pd.to_numeric(c.net_return_pct,errors='coerce')
    summary={'total_samples':len(out),'completed_samples':len(c),'not_completed_samples':int((out.status!='completed').sum()),'profit_samples':int((c.net_return_pct>0).sum()),'non_loss_samples':int((c.net_return_pct>=0).sum()),'loss_samples':int((c.net_return_pct<0).sum()),'win_rate_pct':float((c.net_return_pct>0).mean()*100) if len(c) else float('nan'),'non_loss_rate_pct':float((c.net_return_pct>=0).mean()*100) if len(c) else float('nan'),'average_net_return_pct':float(c.net_return_pct.mean()) if len(c) else float('nan'),'median_net_return_pct':float(c.net_return_pct.median()) if len(c) else float('nan'),'max_net_return_pct':float(c.net_return_pct.max()) if len(c) else float('nan'),'min_net_return_pct':float(c.net_return_pct.min()) if len(c) else float('nan')}
    ...
    print(out[['signal_date','stock_name','status','entry_source','t1_minute_source','net_return_pct','detail']].to_string(index=False))
